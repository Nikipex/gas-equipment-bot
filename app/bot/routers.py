...
from app.bot.handlers import ai_intent, price, price_list, quote, search, start, stock, supplier_files, equipment_selector, supplier_sites, search_modes, thinking, knowledge_upload, alternatives

main_router = Router()

# ВАЖНО: режимовые кнопки выше обычного поиска
main_router.include_router(search_modes.router)
main_router.include_router(alternatives.router)
main_router.include_router(thinking.router)
main_router.include_router(knowledge_upload.router)
